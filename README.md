...
二采的“二采使用上下文”开启时，会把上一段最终二采视频/音频的尾部窗口重新适配到二采目标 latent 尺寸后再注入；不会使用上一段一采结果。关闭时不会第二次注入 Motion Context，二采直接从已包含一采接续边界的 latent 开始。测试表明关闭二采上下文时无法稳定保持跨段一致性；需要二采继续参考上一段时应开启该选项。建议二采降噪 `0.45～0.65`。

视频上下文接续使用 H3 Guide 在当前片段时间轴的开头直接约束上一段尾部窗口，不会把上一段作为额外前缀写入本段，也不会默认裁剪本段开头。因此拼接时不会因插件重复保存上一段画面；边界处出现极短的相近姿态是模型为保持动作连续性生成的过渡，而非重复复制帧。此前实验性的“二采局部时间遮罩”和“二采残差混合强度”已完全移除；关闭“二采使用上下文”时，第二阶段直接从已完成一采的 latent 开始，不再额外锁定或混合接缝。需要更强的跨段一致性时，应开启“二采使用上下文”。
