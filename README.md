...
- `H3 自动导演｜项目计划`：保存片段提示词、时长、参考素材、连续生成、统一输出文件名、防覆盖及缓存设置。
- `H3 自动导演｜TTS 项目计划`：按片段生成和保存音频；可独立控制音频接续和最终音频拼接。
- `H3 自动导演｜动作迁移项目计划`：按参考动作视频切分片段，视频可选择仅用于动作/时长计算或同时作为多模态参考。
- `H3 自动导演｜片段设置`、`H3 自动导演｜多模态素材解析`、`H3 自动导演｜提示词与素材缓存`：为当前片段准备提示词、参考图像/视频/音频和缓存条件（支持节点尺寸记忆）。
- `H3 自动导演｜上下文读取`、`H3 自动导演｜断点续接`、`H3 自动导演｜运动上下文`：读取项目中的视频、音频或一采/二采独立 AV latent 上下文；运动上下文简化为潜空间直取。
- `H3 自动导演｜双阶段采样`：支持一采与二采独立上下文接续（一采接续上一段一采，二采接续上一段二采）；可关闭二采；二采模型未连接时复用一采模型；分辨率相同且网格一致时直接复用一采 latent。
- `H3 自动导演｜双采样分辨率`：输出两阶段对齐后的尺寸和分辨率配置。
- `H3 自动导演｜一采/二采模型加载`、`H3 自动导演｜多模态参考模型加载`、`H3 自动导演｜动作迁移模型加载`：选择 H3 模型，可选 FL2VA/Ref2VA 混合。
- `H3 自动导演｜AV 解码`、`H3 视频迁移｜按策略解码`、`H3 TTS｜仅解码音频`、`H3 自动导演｜保存片段`：解码和保存生成结果（支持按子批次自动防覆盖隔离）。
- `H3 自动导演｜拼接最终视频`、`H3 TTS｜拼接最终音频`：从磁盘片段完成最终输出（支持同名自动递增编号防覆盖）。
...
- **统一输出文件名与防覆盖机制**：
  - 项目计划节点中提供统一的 `输出文件名`（留空默认 `H3`）和 `是否覆盖已有文件` 选项。
  - **中间片段嵌套隔离**：当关闭覆盖时，每次新批次会在 `clips/`、`cache/`、`context/` 下自动创建递增编号的子文件夹（如 `clips/H3/`，已存在则递增为 `clips/H3_1/`、`clips/H3_2/` 等），互不干扰。读取上下文或断点续接时，如果存在多个同名前缀子文件夹，系统默认读取**数字后缀最大**的子文件夹。
  - **最终视频防覆盖**：当关闭覆盖时，若 `final/` 下已存在同名视频，自动追加递增序号（如 `H3_1.mp4`、`H3_2.mp4`）。
- **两阶段潜空间独立接续**：
  - 一采接续严格使用上一片段的一采潜空间（`stage1_latent`），二采接续严格使用上一片段的二采潜空间（`latent`），确保采样状态与分辨率对齐一致。
  - `H3 自动导演｜上下文读取` 提供独立的 `二采上下文潜变量` 输出端口，便于直连双阶段采样节点。
  - `H3 自动导演｜运动上下文` 精简为仅保留“潜空间直取”方案。
...
- **统一参考集潜空间缓存复用**：开启「统一参考集」后，第 1 段编码完成的参考图片、视频、音频的 VAE 潜空间（`ref_blocks`）和分词数据自动保留在内存缓存池中；后续第 2~N 段直接复用该潜空间缓存，仅需按当前段时长生成空潜空间，完全跳过耗时的重复 VAE 编码；若某段启用了“上片段视频参考”，系统仅对上一段新生成的视频做增量编码并自动组合，大幅提升整体多段生成速度并节省显存。
