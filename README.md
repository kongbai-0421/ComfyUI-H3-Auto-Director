...
需要跨 ComfyUI 重启复用时，可开启“缓存提示词向量到硬盘”。该开关可以单独使用；若同时开启“一次性缓存提示词向量”，首次运行会预编码所有当前可用片段，之后在内存和硬盘两级复用。向量保存在当前项目的 `cache/prompt_embeddings/segment_00001.pt` 等文件，`manifest.json` 记录项目参数、分辨率、参考图缩放模式、编码器类型、每段提示词和参考素材文件的大小/修改时间。下一次执行会逐段比对清单，匹配则加载 CPU 向量，只有不匹配的片段重新编码。
...
- 开启“一次性缓存提示词向量”后，所有片段的多模态 conditioning 只编码一次；开启“缓存提示词向量到硬盘”后可跨重启保存并按清单逐段复用，两个开关可以独立使用。清单 JSON 会按片段提示词、素材文件指纹、分辨率和编码器配置决定是否重新编码。随机种子不会使缓存失效。
...
二采的“二采使用上下文”开启时，会把上一段最终二采视频/音频的尾部窗口重新适配到二采目标 latent 尺寸后再注入；不会使用上一段一采结果。关闭时不会第二次注入 Motion Context，但二采仍从已包含一采接续边界的 latent 开始，插件会在二采完成后锁回该上下文前缀（视频并同步锁定对应音频开头），因此只细化后续内容而不重写片段接缝。建议二采降噪 `0.45～0.65`；若希望二采也重新响应上一段动作，再开启“二采使用上下文”。
