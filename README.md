...
`shift_video` 和 `shift_audio` 默认保持 `12.00 / 3.00`。每次只使用一个音频采样切换节点；它输出“音频采样配置”，其中包含所选方法、视频偏移和音频偏移。把该输出连接到双采样节点的“音频采样配置”输入，模型主链仍直接接入双采样节点。
节点还分别输出“视频调度偏移”和“音频调度偏移”两个 `FLOAT` 值，便于其他节点复用基础偏移参数。标准 `BasicGuider` / `BasicScheduler` 工作流不经过双采样节点时，在模型链后接 `H3 自动导演｜应用音频采样配置`，再把采样切换节点的配置输出接入它。
该节点不提供、计算或输出调度器、采样步数、降噪和 `SIGMAS`。一采/二采的调度器、步数与降噪完全由双采样节点本身控制，因此一采设为 20 步就会实际使用 20 步。连接高级工作流自行生成的外部 `SIGMAS` 时，外部调度仍按双采样节点的标准接口优先。
