...
`MiniMax H3 多模态参考生成｜提示词缓存` 提供两种图片参考尺寸模式：自动模式通过 `match/max` 下拉框切换，手动模式通过“使用手动参考最短边”开关启用。启用手动模式时自动下拉框隐藏，关闭手动模式后自动下拉框恢复，不会强制改写另一个控件的值；手动输入会立即对齐并显示为最近的 32 倍数，后端执行时也会再次校正。图片只缩小不放大，视频参考仍使用 H3 自己的约 768 短边/面积上限适配规则。
