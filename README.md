...
该节点不提供、计算或输出采样步数、调度器或降噪。“SIGMAS”输出仅提供端点，连接“插值扩展 Sigmas”后由该扩展节点的 `steps` 决定最终步数；一采/二采的步数与降噪仍由双采样节点本身控制，因此一采设为 20 步就会实际使用 20 步。只有把外部或扩展后的完整 `SIGMAS` 明确接入双采样的一采/二采 Sigma 输入时，才会覆盖对应阶段的调度。
