...
视频上下文接续会额外生成 16 帧隐藏预热画面，并在保存中间片段前与上下文头部一起裁掉，不改变最终可见片段的目标时长；连接 Motion Context 的“裁剪帧数”输出时会自动包含这 16 帧。此前实验性的“二采局部时间遮罩”和“二采残差混合强度”已移除；关闭“二采使用上下文”时，第二阶段直接从一采放大后的 latent 开始，不再额外锁定或混合接缝。需要更强的跨段一致性时，应开启“二采使用上下文”。
