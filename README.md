...
`shift_video` 和 `shift_audio` 默认保持 `12.00 / 3.00`。每次只使用一个音频采样切换节点；它只输出一个“SIGMAS 调度信息”接口，其中包含所选采样方法、视频偏移和音频偏移。把该输出连接到双采样节点的“SIGMAS 调度信息”输入，模型主链仍直接接入双采样节点。
标准 `BasicGuider` / `BasicScheduler` 工作流不经过双采样节点时，在模型链后接 `H3 自动导演｜应用音频采样配置`，再把该节点唯一的“SIGMAS 调度信息”输出接入它。
