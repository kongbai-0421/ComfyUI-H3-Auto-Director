...
把它放在 UNET、LoRA 等模型补丁之后。`shift_video` 和 `shift_audio` 默认保持
`12.00 / 3.00`。每次只使用一个音频采样切换节点；节点从加速/注意力优化后的模型分支读取 `MODEL`，仅输出 `SIGMAS`。模型主链直接接入双采样节点，`SIGMAS` 输出接入双采样节点的一采调度输入。
该节点不再显示调度器、采样步数和降噪控件，内部使用稳定的 `simple / 8 步 / 1.0` 调度。该节点只生成与所选模式对应的 `SIGMAS`，不会替换双采样节点收到的 `MODEL`，因此模型主链不应再串联核心
`ModelSamplingMiniMaxH3` 或另一个 Legacy 节点。工作流文件不会因安装插件而自动修改。
节点名称为“音频采样切换”，两个模式只改变 H3 音频采样时间尺度，视频 `shift_video` 仍由同一节点控制。节点的 `MODEL` 输入仅用于根据所选采样方式计算 `SIGMAS`，实际模型主链仍直接连接双采样节点；将 `SIGMAS` 接到双采样节点的一采输入即可，二采未单独连接时会自动复用一采调度。
